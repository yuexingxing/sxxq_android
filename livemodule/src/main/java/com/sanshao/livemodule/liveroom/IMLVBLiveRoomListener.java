...
public interface IMLVBLiveRoomListener {
     ...
     * 收到新主播进房通知
     *
     * 房间内的主播（和连麦中的观众）会收到新主播的进房事件，您可以调用 {@link MLVBLiveRoom(AnchorInfo, TXCloudVideoView, PlayCallback)} 显示该主播的视频画面。
     *
     ...
     * 收到主播退房通知
     *
     * 房间内的主播（和连麦中的观众）会收到新主播的退房事件，您可以调用 {@link MLVBLiveRoom(AnchorInfo)} 关闭该主播的视频画面。
     *
     ...
     *
     * 主播收到其他房间主播的 PK 请求
     * 如果同意 PK ，您需要调用 {@link MLVBLiveRoom(AnchorInfo, TXCloudVideoView, PlayCallback)}  接口播放邀约主播的流
     *
